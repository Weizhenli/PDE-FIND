...
def PolyDiff(u, x, deg = 3, diff = 1, width = 5):
    deg = int(deg); diff = int(diff); width = int(width)
    """
    ...
    for j in range(width, n-width):
        ...
        points = np.arange(j - width, j + width) # this is incorrect # points = np.arange(j - width, j + width) # this is correct

        # Fit to a Chebyshev polynomial
        # this is the same as any polynomial since we're on a fixed grid but it's better conditioned :)
        poly = np.polynomial.chebyshev.Chebyshev.fit(x[points],u[points],int(deg))
    ...
    dx = x[2]-x[1]
    for d in range(1,diff+1):
        du0 = ss.savgol_filter(u, 2*width+1, deg, d)/(dx**d)
        du[:, d-1] = du0[width:(n-width)]
    '''
    return du
...
def PolyDiffPoint(u, x, deg = 3, diff = 1, index = None):
    ...
    diff = maximum order derivative we want
    """
    #oiu=[int(i) for i in np.arange(t-(Nt-1)/2,t+(Nt+1)/2)]
    #u=Wn[x,y,int(t-(N-1)/2):int(t+(N+1)/2)]; x=np.arange(N)*dt; deg= deg; diff=1; index=None
    n = len(x)
    if index == None: index = int((n-1)/2)

    # Fit to a Chebyshev polynomial
    # better conditioned than normal polynomials
    poly = np.polynomial.chebyshev.Chebyshev.fit(x,u,int(deg))
...
def TrainSTRidge(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    ...
    err_best = np.linalg.norm(TestY - TestR.dot(w_best), 2)**2 + l0_penalty*np.count_nonzero(w_best)
    tol_best = 0
    ...
    for iter in range(maxit):
        ...
        err = np.linalg.norm(TestY - TestR.dot(w), 2)**2 + l0_penalty*np.count_nonzero(w)
